...
Matrix::Matrix(const vector<vector<double>> &matr, int rows, int cols) : rows_(rows), cols_(cols)
{
    allocSpace();
    for (int i = 0; i < rows_; ++i)
    {
...
Matrix Matrix::solve(Matrix A, Matrix b)
{
    ...
    for (int i = 0; i < A.rows_; ++i)
    {
        if (A.p[i][i] == 0)
        {
            // pivot 0 - throw error
            throw domain_error(
                "Error: the coefficient matrix has 0 as a pivot. Please fix the input and try "
                "again.");
        }
